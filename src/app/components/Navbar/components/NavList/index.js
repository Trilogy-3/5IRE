...
function ChildComponent({ item, index, openIndex, handleDropdownToggle, dropdownRef }) {
    ...
    return (
        <div
            ...
            onClick={handleClick}
        >
           <p className="hover:bg-boxColor hover:text-sky-700 rounded-xl flex flex-row items-center  p-3 gap-1 "> {item.name} <Down className="text-lg" /></p>
            {openIndex === index && (
                <div 
                    ref={dropdownRef} 
                ...
                >
                    {item.children && item.children.map((item,index) => {
                        return (
                            <Link href={ item.href} className="flex flex-row items-center gap-2 text-nowrap px-5  py-2   hover:text-white hover:bg-blue-400 ">{ item.name} {item.isOtherPage?<SideArrow/>:''} </Link>
                         )
                     }) }
                </div>
            )}
